...
def lineup_df(self):
        ...
        p_df.loc[(pd_df['batters_faced_vr'] < 100) | (p_df['ra-_b_vr'].isna()), 'ra-_b_vr'] = p_q_vr['ra-_b_vr'].median()
        p_df.loc[(pd_df['batters_faced_vl'] < 100) | (p_df['ra-_b_vl'].isna()), 'ra-_b_vl'] = p_q_vl['ra-_b_vl'].median()
        ...
        p_df['exp_inn'] = (pd_df['exp_bf'].max() - p_df['exp_ra'].max()) / 3
        if self.is_home:
        ...
        h_df['exp_bp_pa'] = 0
        while exp_bf_bp > 0:
            h_df.loc[h_df['order'] == order, 'exp_bp_pa'] += 1
            order += 1
            if order == 9:
                order = 1
            exp_bf_bp -+ 1
        h_df['exp_ps_bp'] = h_df['exp_bp_pa'] * bp['fd_wpa_b_rp'].mean()
    ...
    sp_rollover = floor((p_df['exp_x_lu'] % 1) * 9)
        ...
        h_df.loc[h_df['order'] > sp_rollover, 'exp_pa_sp'] = floor(p_df['exp_x_lu'])
    ra-_b_vl
    total_righties = len(h_df.loc[righties].index)
        total_lefties = len(h_df.loc[lefties].index)
        
        exp_bf_bp = floor((exp_bp_inn * 3) + ((exp_bp_inn * 3) * bp['ra-_b_rp']))
...
def sp_df(self):
        ...
        p_df['pitches_start'].fillna(p_q_sp['pitches_start'].median(), inplace = True)
        key = 'pitches_pa_' + self.opp_instance.o_split
        p_df['exp_x_lu'] = p_df['pitches_start'] / h_df[key].sum()
        p_df['exp_bf'] = round((p_df['exp_x_lu'] * 9))
