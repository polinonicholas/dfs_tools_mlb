...
if not data_path.exists():
    def split_fd_pts(df):
        ...
        df['fd_points+'] = ((df['single'] * b) + \
                        ...
                        (df['runs'] * r) + \
                        (df['rbi'] * rbi))
        df['fd_points_lhb'] = ((df['single_lhb'] * b) + \
                        (df['walk_lhb'] * b) + \
                        (df['double_lhb'] * (b*2)) +  \
                        (df['triple_lhb'] * (b*3)) + \
                        (df['home_run_lhb'] * ((b*4) + r)))
        df['fd_points_rhb'] = ((df['single_rhb'] * b) + \
                        (df['walk_rhb'] * b) + \
                        (df['double_rhb'] * (b*2)) +  \
                        (df['triple_rhb'] * (b*3)) + \
    ...
    game_data.loc[game_data['condition'].isin(mac.weather.roof_closed), 'temp'] = settings.INDOOR_TEMP
    game_data.loc[game_data['condition'].isin(mac.weather.roof_closed), 'wind_speed'] = settings.INDOOR_WIND
    game_data['date'] = pd.to_datetime(game_data['date'], infer_datetime_format=True)
    def split_columns(split='RHB', event_type='success'):
        all_split = ('vs_' + split.upper() + '_' + x for x in mac.event_types[event_type])
    ...
    stolen_base = [x for x in game_data.columns if x in mac.event_types.stolen_base.success]
    caught_stealing = [x for x in game_data.columns if x in mac.event_types.stolen_base.fail]
    ...
    game_data['fail_lhb'] = game_data[lhb_fail].sum(axis=1, skipna=True, numeric_only=True)
