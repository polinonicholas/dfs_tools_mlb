...
if not data_path_h.exists():
    ...
    h_splits["1b_vl"] = h_splits["hits_vl"] - (
        h_splits["hr_vl"] + h_splits["3b_vl"] + h_splits["2b_vl"]
    )
    h_splits["1b_vr"] = h_splits["hits_vr"] - (
        h_splits["hr_vr"] + h_splits["3b_vr"] + h_splits["2b_vr"]
    )
    ...
    h_splits["total_pitches"] = (
        h_splits["total_pitches_vl"] + h_splits["total_pitches_vr"]
    )
    ...
    h_splits["bases+_vr"] = (
        h_splits["total_bases_vr"] + h_splits["bb_vr"] + h_splits["hbp_vr"]
    )
    ...
    h_splits["bases+_vl"] = (
        h_splits["total_bases_vl"] + h_splits["bb_vl"] + h_splits["hbp_vl"]
    )
    ...
    h_splits["fd_bb_weight_vr"] = (
        (h_splits["bb_vr"] * b) + ((h_splits["bb_vr"] * weight_bb) * r)
    ) / h_splits["pa_vr"]
    h_splits["fd_hbp_weight_vr"] = (
        (h_splits["hbp_vr"] * b) + ((h_splits["hbp_vr"] * weight_hbp) * x)
    ) / h_splits["pa_vr"]
    h_splits["fd_1b_weight_vr"] = (
        (h_splits["1b_vr"] * b) + ((h_splits["1b_vr"] * weight_1b) * x)
    ) / h_splits["pa_vr"]
    h_splits["fd_2b_weight_vr"] = (
        (h_splits["2b_vr"] * (b * 2)) + ((h_splits["2b_vr"] * weight_2b) * x)
    ) / h_splits["pa_vr"]
    h_splits["fd_3b_weight_vr"] = (
        (h_splits["3b_vr"] * (b * 3)) + ((h_splits["3b_vr"] * weight_3b) * x)
    ) / h_splits["pa_vr"]
    h_splits["fd_hr_weight_vr"] = (
        (h_splits["hr_vr"] * 18.7) + ((h_splits["hr_vr"] * (weight_hr - 1)) * rbi)
    ) / h_splits["pa_vr"]
    h_splits["fd_sb_weight_vr"] = (
        (h_splits["sb_vr"] * s) + ((h_splits["sb_vr"] * weight_sb) * r)
    ) / h_splits["pa_vr"]
    # weighted points/pa - vs. right
    h_splits["fd_wps_pa_vr"] = (
        h_splits["fd_bb_weight_vr"]
        + h_splits["fd_hbp_weight_vr"]
        + h_splits["fd_1b_weight_vr"]
        + h_splits["fd_2b_weight_vr"]
    ...
    h_splits["fd_hbp_weight_vl"] = (
        (h_splits["hbp_vl"] * b) + ((h_splits["hbp_vl"] * weight_hbp) * x)
    ) / h_splits["pa_vl"]
    h_splits["fd_1b_weight_vl"] = (
        (h_splits["1b_vl"] * b) + ((h_splits["1b_vl"] * weight_1b) * x)
    ) / h_splits["pa_vl"]
    h_splits["fd_2b_weight_vl"] = (
        (h_splits["2b_vl"] * (b * 2)) + ((h_splits["2b_vl"] * weight_2b) * x)
    ) / h_splits["pa_vl"]
    h_splits["fd_3b_weight_vl"] = (
        (h_splits["3b_vl"] * (b * 3)) + ((h_splits["3b_vl"] * weight_3b) * x)
    ) / h_splits["pa_vl"]
    h_splits["fd_hr_weight_vl"] = (
        (h_splits["hr_vl"] * 18.7) + ((h_splits["hr_vl"] * (weight_hr - 1)) * rbi)
    ) / h_splits["pa_vl"]
    h_splits["fd_sb_weight_vl"] = (
        (h_splits["sb_vl"] * s) + ((h_splits["sb_vl"] * weight_sb) * r)
    ) / h_splits["pa_vl"]
    # weighted points/pa - vs. left
    h_splits["fd_wps_pa_vl"] = (
        h_splits["fd_bb_weight_vl"]
        + h_splits["fd_hbp_weight_vl"]
        + h_splits["fd_1b_weight_vl"]
        + h_splits["fd_2b_weight_vl"]
    ...
    h_splits["fd_bb_weight"] = (
        (h_splits["bb"] * b) + ((h_splits["bb"] * weight_bb) * r)
    ) / h_splits["pa"]
    h_splits["fd_hbp_weight"] = (
        (h_splits["hbp"] * b) + ((h_splits["hbp"] * weight_hbp) * x)
    ) / h_splits["pa"]
    h_splits["fd_1b_weight"] = (
        (h_splits["1b"] * b) + ((h_splits["1b"] * weight_1b) * x)
    ) / h_splits["pa"]
    h_splits["fd_2b_weight"] = (
        (h_splits["2b"] * (b * 2)) + ((h_splits["2b"] * weight_2b) * x)
    ) / h_splits["pa"]
    h_splits["fd_3b_weight"] = (
        (h_splits["3b"] * (b * 3)) + ((h_splits["3b"] * weight_3b) * x)
    ) / h_splits["pa"]
    h_splits["fd_hr_weight"] = (
        (h_splits["hr"] * 18.7) + ((h_splits["hr"] * (weight_hr - 1)) * rbi)
    ) / h_splits["pa"]
    h_splits["fd_sb_weight"] = (
        (h_splits["sb"] * s) + ((h_splits["sb"] * weight_sb) * r)
    ) / h_splits["pa"]
    ...
    # points/pa - vs. right
    h_splits["fd_ps_pa_vr"] = (
        (h_splits["bases+_vr"] * b)
        + (h_splits["sb_vr"] * s)
        + (h_splits["hr_vr"] * (r + rbi))
    ) / h_splits["pa_vr"]
    # points/pa - vs. left
    h_splits["fd_ps_pa_vl"] = (
        (h_splits["bases+_vl"] * b)
        + (h_splits["sb_vl"] * s)
        + (h_splits["hr_vl"] * (r + rbi))
    ) / h_splits["pa_vl"]
    # points/pa
    h_splits["fd_ps_pa"] = (
        (h_splits["bases+"] * b) + (h_splits["sb"] * s) + (h_splits["hr"] * (r + rbi))
    ) / h_splits["pa"]
    # weighted points conceded/pa - right
    h_splits["fd_wpa_pa_vr"] = (
        ((h_splits["k_vr"] * k) + (h_splits["outs-_vr"] * o))
        + ((h_splits["hr_vr"] * weight_hr) * er)
        + ((h_splits["3b_vr"] * weight_3b) * er)
        + ((h_splits["2b_vr"] * weight_2b) * er)
        + ((h_splits["1b_vr"] * weight_1b) * er)
        + ((h_splits["bb_vr"] * weight_bb) * er)
        + ((h_splits["hbp_vr"] * weight_hbp) * er)
        + ((h_splits["sb_vr"] * weight_sb) * er)
    ) / h_splits["pa_vr"]
    # weighted points conceded/pa - left
    h_splits["fd_wpa_pa_vl"] = (
        ((h_splits["k_vl"] * k) + (h_splits["outs-_vl"] * o))
        + ((h_splits["hr_vl"] * weight_hr) * er)
        + ((h_splits["3b_vl"] * weight_3b) * er)
        + ((h_splits["2b_vl"] * weight_2b) * er)
        + ((h_splits["1b_vl"] * weight_1b) * er)
        + ((h_splits["bb_vl"] * weight_bb) * er)
        + ((h_splits["hbp_vl"] * weight_hbp) * er)
        + ((h_splits["sb_vl"] * weight_sb) * er)
    ) / h_splits["pa_vl"]
    # weighted points conceded/pa
    h_splits["fd_wpa_pa"] = (
        ((h_splits["k"] * k) + (h_splits["outs-"] * o))
        + ((h_splits["hr"] * weight_hr) * er)
        + ((h_splits["3b"] * weight_3b) * er)
        + ((h_splits["2b"] * weight_2b) * er)
        + ((h_splits["1b"] * weight_1b) * er)
        + ((h_splits["bb"] * weight_bb) * er)
        + ((h_splits["hbp"] * weight_hbp) * er)
        + ((h_splits["sb"] * weight_sb) * er)
    ) / h_splits["pa"]

    # points conceded/pa vs. right
    h_splits["fd_pa_pa_vr"] = (
        (h_splits["k_vr"] * k) + (h_splits["outs-_vr"] * o) + (h_splits["hr_vr"] * er)
    ) / h_splits["pa_vr"]
    # points conceded/pa vs. left
    h_splits["fd_pa_pa_vl"] = (
        (h_splits["k_vl"] * k) + (h_splits["outs-_vl"] * o) + (h_splits["hr_vl"] * er)
    ) / h_splits["pa_vl"]
    # points conceded/pa
    h_splits["fd_pa_pa"] = (
        (h_splits["k"] * k) + (h_splits["outs-"] * o) + (h_splits["hr"] * er)
    ) / h_splits["pa"]
    ...
    platoon_filt = (
        (h_splits["pa_vr"] / h_splits["pa_vl"] > (vr_vl_pa_ratio * 2))
        | (h_splits["pa_vl"] / h_splits["pa_vr"] > (vl_vr_pa_ratio * 2))
    ) & (h_splits["pa"] > h_splits["pa"].median())
...
if not data_path_p.exists():
    ...
    p_splits["1b_vr"] = p_splits["hits_vr"] - (
        p_splits["hr_vr"] + p_splits["3b_vr"] + p_splits["2b_vr"]
    )
    p_splits["1b_sp"] = p_splits["hits_sp"] - (
        p_splits["hr_sp"] + p_splits["3b_sp"] + p_splits["2b_sp"]
    )
    p_splits["1b_rp"] = p_splits["hits_rp"] - (
        p_splits["hr_rp"] + p_splits["3b_rp"] + p_splits["2b_rp"]
    )
    ...
    try:
        ...
        p_splits["complete_games"] = (
            p_splits["complete_games_21"] + p_splits["complete_games_22"]
        )
    ...
    p_splits["total_pitches"] = (
        p_splits["total_pitches_vr"] + p_splits["total_pitches_vl"]
    )
    ...
    p_splits["batters_faced"] = (
        p_splits["batters_faced_vr"] + p_splits["batters_faced_vl"]
    )
    ...
    p_splits["ra-_b"] = (
        p_splits["runners_allowed"] - (p_splits["gidp"] + p_splits["cs+"])
    ) / p_splits["batters_faced"]
    ...
    p_splits["runners_allowed_sp"] = (
        p_splits["hits_sp"] + p_splits["bb_sp"] + p_splits["hb_sp"]
    )
    ...
    p_splits["bases+_sp"] = (
        p_splits["total_bases_sp"] + p_splits["bb_sp"] + p_splits["hb_sp"]
    )
    p_splits["sb+_sp"] = (
        p_splits["sb_sp"] + p_splits["wild_pitches_sp"] + p_splits["balks_sp"]
    )
    p_splits["cs+_sp"] = p_splits["cs_sp"] + p_splits["pickoffs_sp"]
    p_splits["ra-_b_sp"] = (
        p_splits["runners_allowed_sp"] - (p_splits["gidp_sp"] + p_splits["cs+_sp"])
    ...
    p_splits["runners_allowed_rp"] = (
        p_splits["hits_rp"] + p_splits["bb_rp"] + p_splits["hb_rp"]
    )
    ...
    p_splits["bases+_rp"] = (
        p_splits["total_bases_rp"] + p_splits["bb_rp"] + p_splits["hb_rp"]
    )
    p_splits["sb+_rp"] = (
        p_splits["sb_rp"] + p_splits["wild_pitches_rp"] + p_splits["balks_rp"]
    )
    p_splits["cs+_rp"] = p_splits["cs_rp"] + p_splits["pickoffs_rp"]
    p_splits["ra-_b_rp"] = (
        p_splits["runners_allowed_rp"] - (p_splits["gidp_rp"] + p_splits["cs+_rp"])
    ...
    p_splits["runners_allowed_vr"] = (
        p_splits["hits_vr"] + p_splits["bb_vr"] + p_splits["hb_vr"]
    )
    ...
    p_splits["bases+_vr"] = (
        p_splits["total_bases_vr"] + p_splits["bb_vr"] + p_splits["hb_vr"]
    )
    p_splits["sb+_vr"] = (
        p_splits["sb_vr"] + p_splits["wild_pitches_vr"] + p_splits["balks_vr"]
    )
    p_splits["cs+_vr"] = p_splits["cs_vr"] + p_splits["pickoffs_vr"]
    p_splits["ra-_b_vr"] = (
        p_splits["runners_allowed_vr"] - (p_splits["gidp_vr"] + p_splits["cs+_vr"])
    ...
    p_splits["runners_allowed_vl"] = (
        p_splits["hits_vl"] + p_splits["bb_vl"] + p_splits["hb_vl"]
    )
    ...
    p_splits["bases+_vl"] = (
        p_splits["total_bases_vl"] + p_splits["bb_vl"] + p_splits["hb_vl"]
    )
    p_splits["sb+_vl"] = (
        p_splits["sb_vl"] + p_splits["wild_pitches_vl"] + p_splits["balks_vl"]
    )
    p_splits["cs+_vl"] = p_splits["cs_vl"] + p_splits["pickoffs_vl"]
    p_splits["ra-_b_vl"] = (
        p_splits["runners_allowed_vl"] - (p_splits["gidp_vl"] + p_splits["cs+_vl"])
    ) / p_splits["batters_faced_vl"]

    # weighted by catergoy/batter faced - right
    p_splits["fd_bb_weight_vr"] = ((p_splits["bb_vr"] * weight_bb) * er) / p_splits[
        "batters_faced_vr"
    ]
    p_splits["fd_hb_weight_vr"] = ((p_splits["hb_vr"] * weight_hbp) * er) / p_splits[
        "batters_faced_vr"
    ]
    p_splits["fd_1b_weight_vr"] = ((p_splits["1b_vr"] * weight_1b) * er) / p_splits[
        "batters_faced_vr"
    ]
    p_splits["fd_2b_weight_vr"] = ((p_splits["2b_vr"] * weight_2b) * er) / p_splits[
        "batters_faced_vr"
    ]
    p_splits["fd_3b_weight_vr"] = ((p_splits["3b_vr"] * weight_3b) * er) / p_splits[
        "batters_faced_vr"
    ]
    p_splits["fd_hr_weight_vr"] = ((p_splits["hr_vr"] * weight_hr) * er) / p_splits[
        "batters_faced_vr"
    ]
    p_splits["fd_sb_weight_vr"] = ((p_splits["sb_vr"] * weight_sb) * er) / p_splits[
        "batters_faced_vr"
    ]
    p_splits["fd_sb+_weight_vr"] = ((p_splits["sb+_vr"] * weight_sb) * er) / p_splits[
        "batters_faced_vr"
    ]
    p_splits["fd_balk_weight_vr"] = (
        (p_splits["balks_vr"] * weight_sb) * er
    ) / p_splits["batters_faced_vr"]
    p_splits["fd_wp_weight_vr"] = (
        (p_splits["wild_pitches_vr"] * weight_sb) * er
    ) / p_splits["batters_faced_vr"]
    p_splits["fd_cs_weight_vr"] = ((p_splits["cs_vr"] * weight_cs) * er) / p_splits[
        "batters_faced_vr"
    ]
    ...
    p_splits["fd_wps_b_vr"] = (
        p_splits["fd_bb_weight_vr"]
        + p_splits["fd_hb_weight_vr"]
        + p_splits["fd_1b_weight_vr"]
        + p_splits["fd_2b_weight_vr"]
        + p_splits["fd_3b_weight_vr"]
        + p_splits["fd_hr_weight_vr"]
        + p_splits["fd_sb+_weight_vr"]
        + p_splits["fd_cs_weight_vr"]
        + p_splits["fd_ip_weight_vr"]
        + p_splits["fd_k_weight_vr"]
    )

    # weighted by catergoy/batter faced - left
    p_splits["fd_bb_weight_vl"] = ((p_splits["bb_vl"] * weight_bb) * er) / p_splits[
        "batters_faced_vl"
    ]
    p_splits["fd_hb_weight_vl"] = ((p_splits["hb_vl"] * weight_hbp) * er) / p_splits[
        "batters_faced_vl"
    ]
    p_splits["fd_1b_weight_vl"] = ((p_splits["1b_vl"] * weight_1b) * er) / p_splits[
        "batters_faced_vl"
    ]
    p_splits["fd_2b_weight_vl"] = ((p_splits["2b_vl"] * weight_2b) * er) / p_splits[
        "batters_faced_vl"
    ]
    p_splits["fd_3b_weight_vl"] = ((p_splits["3b_vl"] * weight_3b) * er) / p_splits[
        "batters_faced_vl"
    ]
    p_splits["fd_hr_weight_vl"] = ((p_splits["hr_vl"] * weight_hr) * er) / p_splits[
        "batters_faced_vl"
    ]
    p_splits["fd_sb_weight_vl"] = ((p_splits["sb_vl"] * weight_sb) * er) / p_splits[
        "batters_faced_vl"
    ]
    p_splits["fd_sb+_weight_vl"] = ((p_splits["sb+_vl"] * weight_sb) * er) / p_splits[
        "batters_faced_vl"
    ]
    p_splits["fd_balk_weight_vl"] = (
        (p_splits["balks_vl"] * weight_sb) * er
    ) / p_splits["batters_faced_vl"]
    p_splits["fd_wp_weight_vl"] = (
        (p_splits["wild_pitches_vl"] * weight_sb) * er
    ) / p_splits["batters_faced_vl"]
    p_splits["fd_cs_weight_vl"] = ((p_splits["cs_vl"] * weight_cs) * er) / p_splits[
        "batters_faced_vl"
    ]
    ...
    p_splits["fd_wps_b_vl"] = (
        p_splits["fd_bb_weight_vl"]
        + p_splits["fd_hb_weight_vl"]
        + p_splits["fd_1b_weight_vl"]
        + p_splits["fd_2b_weight_vl"]
        + p_splits["fd_3b_weight_vl"]
        + p_splits["fd_hr_weight_vl"]
        + p_splits["fd_sb+_weight_vl"]
        + p_splits["fd_cs_weight_vl"]
        + p_splits["fd_ip_weight_vl"]
        + p_splits["fd_k_weight_vl"]
    )

    # weighted by catergoy/batter faced - starting
    p_splits["fd_bb_weight_sp"] = ((p_splits["bb_sp"] * weight_bb) * er) / p_splits[
        "batters_faced_sp"
    ]
    p_splits["fd_hb_weight_sp"] = ((p_splits["hb_sp"] * weight_hbp) * er) / p_splits[
        "batters_faced_sp"
    ]
    p_splits["fd_1b_weight_sp"] = ((p_splits["1b_sp"] * weight_1b) * er) / p_splits[
        "batters_faced_sp"
    ]
    p_splits["fd_2b_weight_sp"] = ((p_splits["2b_sp"] * weight_2b) * er) / p_splits[
        "batters_faced_sp"
    ]
    p_splits["fd_3b_weight_sp"] = ((p_splits["3b_sp"] * weight_3b) * er) / p_splits[
        "batters_faced_sp"
    ]
    p_splits["fd_hr_weight_sp"] = ((p_splits["hr_sp"] * weight_hr) * er) / p_splits[
        "batters_faced_sp"
    ]
    p_splits["fd_sb_weight_sp"] = ((p_splits["sb_sp"] * weight_sb) * er) / p_splits[
        "batters_faced_sp"
    ]
    p_splits["fd_sb+_weight_sp"] = ((p_splits["sb+_sp"] * weight_sb) * er) / p_splits[
        "batters_faced_sp"
    ]
    p_splits["fd_balk_weight_sp"] = (
        (p_splits["balks_sp"] * weight_sb) * er
    ) / p_splits["batters_faced_sp"]
    p_splits["fd_wp_weight_sp"] = (
        (p_splits["wild_pitches_sp"] * weight_sb) * er
    ) / p_splits["batters_faced_sp"]
    p_splits["fd_cs_weight_sp"] = ((p_splits["cs_sp"] * weight_cs) * er) / p_splits[
        "batters_faced_sp"
    ]
    ...
    p_splits["fd_wps_b_sp"] = (
        p_splits["fd_bb_weight_sp"]
        + p_splits["fd_hb_weight_sp"]
        + p_splits["fd_1b_weight_sp"]
        + p_splits["fd_2b_weight_sp"]
        + p_splits["fd_3b_weight_sp"]
        + p_splits["fd_hr_weight_sp"]
        + p_splits["fd_sb+_weight_sp"]
        + p_splits["fd_cs_weight_sp"]
        + p_splits["fd_ip_weight_sp"]
        + p_splits["fd_k_weight_sp"]
    )

    # weighted by catergoy/batter faced
    p_splits["fd_bb_weight"] = ((p_splits["bb"] * weight_bb) * er) / p_splits[
        "batters_faced"
    ]
    p_splits["fd_hb_weight"] = ((p_splits["hb"] * weight_hbp) * er) / p_splits[
        "batters_faced"
    ]
    p_splits["fd_1b_weight"] = ((p_splits["1b"] * weight_1b) * er) / p_splits[
        "batters_faced"
    ]
    p_splits["fd_2b_weight"] = ((p_splits["2b"] * weight_2b) * er) / p_splits[
        "batters_faced"
    ]
    p_splits["fd_3b_weight"] = ((p_splits["3b"] * weight_3b) * er) / p_splits[
        "batters_faced"
    ]
    p_splits["fd_hr_weight"] = ((p_splits["hr"] * weight_hr) * er) / p_splits[
        "batters_faced"
    ]
    p_splits["fd_sb_weight"] = ((p_splits["sb"] * weight_sb) * er) / p_splits[
        "batters_faced"
    ]
    p_splits["fd_sb+_weight"] = ((p_splits["sb+"] * weight_sb) * er) / p_splits[
        "batters_faced"
    ]
    p_splits["fd_balk_weight"] = ((p_splits["balks"] * weight_sb) * er) / p_splits[
        "batters_faced"
    ]
    p_splits["fd_wp_weight"] = ((p_splits["wild_pitches"] * weight_sb) * er) / p_splits[
        "batters_faced"
    ]
    p_splits["fd_cs_weight"] = ((p_splits["cs"] * weight_cs) * er) / p_splits[
        "batters_faced"
    ]
    ...
    p_splits["fd_wps_b"] = (
        p_splits["fd_bb_weight"]
        + p_splits["fd_hb_weight"]
        + p_splits["fd_1b_weight"]
        + p_splits["fd_2b_weight"]
        + p_splits["fd_3b_weight"]
        + p_splits["fd_hr_weight"]
        + p_splits["fd_sb+_weight"]
        + p_splits["fd_cs_weight"]
        + p_splits["fd_ip_weight"]
        + p_splits["fd_k_weight"]
    )

    # points scored/batter faced - in starts
    p_splits["fd_ps_b_sp"] = (
        (p_splits["ip_sp"] * i) + (p_splits["k_sp"] * k) + (p_splits["er_sp"] * er)
    ) / p_splits["batters_faced_sp"]
    # points scored/start
    p_splits["fd_ps_s"] = (
        p_splits["fd_ps_b_sp"] * p_splits["batters_faced_sp"]
    ) / p_splits["games_sp"]
    # points scored/left-handed batter faced
    p_splits["fd_ps_b_vl"] = (
        (p_splits["ip_vl"] * i) + (p_splits["k_vl"] * k) + (p_splits["er_vl"] * er)
    ) / p_splits["batters_faced_vl"]
    # points score/right-handed batter faced
    p_splits["fd_ps_b_vr"] = (
        (p_splits["ip_vr"] * i) + (p_splits["k_vr"] * k) + (p_splits["er_vr"] * er)
    ) / p_splits["batters_faced_vr"]

    # weight points allowed/batter - vs. right
    p_splits["fd_wpa_b_vr"] = (
        ((p_splits["bb_vr"] * b) + ((p_splits["bb_vr"] * weight_bb) * (r + rbi)))
        + ((p_splits["hb_vr"] * b) + ((p_splits["hb_vr"] * weight_hbp) * (r + rbi)))
        + ((p_splits["1b_vr"] * b) + ((p_splits["1b_vr"] * weight_1b) * (r + rbi)))
        + (
            (p_splits["2b_vr"] * (b * 2))
            + ((p_splits["2b_vr"] * weight_2b) * (r + rbi))
        )
        + (
            (p_splits["3b_vr"] * (b * 3))
            + ((p_splits["3b_vr"] * weight_3b) * (r + rbi))
        )
        + (
            (p_splits["hr_vr"] * hr)
            + ((p_splits["hr_vr"] * (weight_hr - 1)) * (r + rbi))
        )
        + (
            (p_splits["sb_vr"] * s)
            + (((p_splits["sb+_vr"] - p_splits["cs_vr"]) * weight_sb) * (r + rbi))
        )
    ) / p_splits["batters_faced_vr"]

    # weight points allowed/batter - vs. left
    p_splits["fd_wpa_b_vl"] = (
        ((p_splits["bb_vl"] * b) + ((p_splits["bb_vl"] * weight_bb) * (r + rbi)))
        + ((p_splits["hb_vl"] * b) + ((p_splits["hb_vl"] * weight_hbp) * (r + rbi)))
        + ((p_splits["1b_vl"] * b) + ((p_splits["1b_vl"] * weight_1b) * (r + rbi)))
        + (
            (p_splits["2b_vl"] * (b * 2))
            + ((p_splits["2b_vl"] * weight_2b) * (r + rbi))
        )
        + (
            (p_splits["3b_vl"] * (b * 3))
            + ((p_splits["3b_vl"] * weight_3b) * (r + rbi))
        )
        + (
            (p_splits["hr_vl"] * hr)
            + ((p_splits["hr_vl"] * (weight_hr - 1)) * (r + rbi))
        )
        + (
            (p_splits["sb_vl"] * s)
            + (((p_splits["sb+_vl"] - p_splits["cs_vl"]) * weight_sb) * (r + rbi))
        )
    ) / p_splits["batters_faced_vl"]

    # weight points allowed/batter - as starter
    p_splits["fd_wpa_b_sp"] = (
        ((p_splits["bb_sp"] * b) + ((p_splits["bb_sp"] * weight_bb) * (r + rbi)))
        + ((p_splits["hb_sp"] * b) + ((p_splits["hb_sp"] * weight_hbp) * (r + rbi)))
        + ((p_splits["1b_sp"] * b) + ((p_splits["1b_sp"] * weight_1b) * (r + rbi)))
        + (
            (p_splits["2b_sp"] * (b * 2))
            + ((p_splits["2b_sp"] * weight_2b) * (r + rbi))
        )
        + (
            (p_splits["3b_sp"] * (b * 3))
            + ((p_splits["3b_sp"] * weight_3b) * (r + rbi))
        )
        + (
            (p_splits["hr_sp"] * hr)
            + ((p_splits["hr_sp"] * (weight_hr - 1)) * (r + rbi))
        )
        + (
            (p_splits["sb_sp"] * s)
            + (((p_splits["sb+_sp"] - p_splits["cs_sp"]) * weight_sb) * (r + rbi))
        )
    ) / p_splits["batters_faced_sp"]

    # weight points allowed/batter - as reliever
    p_splits["fd_wpa_b_rp"] = (
        ((p_splits["bb_rp"] * b) + ((p_splits["bb_rp"] * weight_bb) * (r + rbi)))
        + ((p_splits["hb_rp"] * b) + ((p_splits["hb_rp"] * weight_hbp) * (r + rbi)))
        + ((p_splits["1b_rp"] * b) + ((p_splits["1b_rp"] * weight_1b) * (r + rbi)))
        + (
            (p_splits["2b_rp"] * (b * 2))
            + ((p_splits["2b_rp"] * weight_2b) * (r + rbi))
        )
        + (
            (p_splits["3b_rp"] * (b * 3))
            + ((p_splits["3b_rp"] * weight_3b) * (r + rbi))
        )
        + (
            (p_splits["hr_rp"] * hr)
            + ((p_splits["hr_rp"] * (weight_hr - 1)) * (r + rbi))
        )
        + (
            (p_splits["sb_rp"] * s)
            + (((p_splits["sb+_rp"] - p_splits["cs_rp"]) * weight_sb) * (r + rbi))
        )
    ) / p_splits["batters_faced_rp"]

    # weight points allowed/batter - as reliever
    p_splits["fd_wpa_b"] = (
        ((p_splits["bb"] * b) + ((p_splits["bb"] * weight_bb) * (r + rbi)))
        ...
        + (
            (p_splits["sb"] * s)
            + (((p_splits["sb+"] - p_splits["cs"]) * weight_sb) * (r + rbi))
        )
    ) / p_splits["batters_faced"]

    # points allowed/start
    p_splits["fd_pa_s"] = (
        (p_splits["bases+_sp"] * b)
        + (p_splits["er_sp"] * (r + rbi))
        + (p_splits["sb_sp"] * s)
    ) / p_splits["games_sp"]
    ...
    p_splits["fd_pa_b_rp"] = (
        (p_splits["bases+_rp"] * b)
        + (p_splits["er_rp"] * (r + rbi))
        + (p_splits["sb_rp"] * s)
    ) / p_splits["batters_faced_rp"]
    ...
    p_splits["fd_pa"] = (
        (p_splits["bases+"] * b) + (p_splits["er"] * (r + rbi)) + (p_splits["sb"] * s)
    )
    # points allowed/ right-handed batter faced
    p_splits["fd_pa_b_vr"] = (
        (p_splits["bases+_vr"] * b)
        + (p_splits["er_vr"] * (r + rbi))
        + (p_splits["sb_vr"] * s)
    ) / p_splits["batters_faced_vr"]
    # points allowed/ left-handed batter faced
    p_splits["fd_pa_b_vl"] = (
        (p_splits["bases+_vl"] * b)
        + (p_splits["er_vl"] * (r + rbi))
        + (p_splits["sb_vl"] * s)
    ) / p_splits["batters_faced_vl"]
...
adjustment_dfs = {
    ...
    "pitchers": {
        "BP": {
            ...
            "RAW": {"vs_l": p_q_vl, "vs_r": p_q_vr, "raw": p_q_rp},
        },
        "SP": {
            "L": {"vs_l": p_q_l_vl, "vs_r": p_q_l_vr},
            "R": {"vs_l": p_q_r_vl, "vs_r": p_q_r_vr},
            "RAW": {"vs_l": p_q_vl, "vs_r": p_q_vr, "raw": p_q_sp},
        },
    },
}
