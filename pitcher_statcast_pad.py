...
def print_p_stats(player_id, year):
    ...
    previous_p = p[((p['date'] <= '2021-06-20') & (p['date'] >= '2021-04-14'))].reset_index()
    ...
    try:
        print('PREVIOUS')
        strikes = previous_p[(previous_p['result'].str.contains('strike')) | 
                          
                          (previous_p['result'].str.contains('_tip') )
                          ]
        print("Strikes:" )
        print(len(strikes.index) / len(previous_p.index))
        extra_base = previous_p[(previous_p['result'].str.contains('home_run')) | 
        ...
        print("K-BB:")
        final_ab_pitch = previous_p[(previous_p['result'].str.contains('strikeout')) |
    ...
    try:
        print('RECENT')
        strikes = recent_p[(recent_p['result'].str.contains('strike')) | 
                          
                          (recent_p['result'].str.contains('_tip') )
                          ]
        print("Strikes:" )
        print(len(strikes.index) / len(recent_p.index))
        extra_base = recent_p[(recent_p['result'].str.contains('home_run')) | 
        ...
        print("K-BB:")
        final_ab_pitch = recent_p[(recent_p['result'].str.contains('strikeout')) |
