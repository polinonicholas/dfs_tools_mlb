...
class Team(metaclass=IterTeam):
    ...
    #                   'team': self.opp_name
    #                       }
    #         df = pd.DataFrame([player]).join(p_splits.set_index('mlb_id'), on='mlb_id', rsuffix='_drop')
    #     except TypeError:
    #         df = Team.default_sp()
    #     df['pitches_start'].fillna(p_q_sp['pitches_start'].median(), inplace = True)
    #     print(df['pitches_start'])
    #     key = 'pitches_pa_' + self.o_split
    #     df['exp_x_lu'] = (df['pitches_start'] / self.lineup_df()[key].sum())
    #     # df['exp_bf'] = round((df['exp_x_lu'].max() * 9))
    #     return df
    @cached_property
