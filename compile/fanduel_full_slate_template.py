...
s = FDSlate(p_fades=[], h_fades=[], salary_batting_order=5, stack_salary_factor=True)
...
].sort_values(by="k_pred", ascending=False)


# get pitcher statcast info for slate
pitcher_statcast_file = pickle_path(
    name=f"pitcher_statcast_{tf.today}_{s.slate_number}", directory=settings.FD_DIR
)
...
if pitcher_statcast_path.exists():
    ...
    for i_d in pitchers["mlb_id"].values:
        try:
            ...
            row = [
                pitchers.loc[pitchers["mlb_id"] == i_d, "name"].item(),
...
first = s.first_df[
    ["name", "points", "exp_ps_sp_pa", "sp_split", "fd_salary"]
].sort_values(by=position_sort_key, ascending=False)
second = s.second_df[
    ["name", "points", "exp_ps_sp_pa", "sp_split", "fd_salary"]
].sort_values(by=position_sort_key, ascending=False)
ss = s.ss_df[["name", "points", "exp_ps_sp_pa", "sp_split", "fd_salary"]].sort_values(
    by=position_sort_key, ascending=False
)
third = s.third_df[
    ["name", "points", "exp_ps_sp_pa", "sp_split", "fd_salary"]
].sort_values(by=position_sort_key, ascending=False)
of = s.of_df[["name", "points", "exp_ps_sp_pa", "sp_split", "fd_salary"]].sort_values(
    by=position_sort_key, ascending=False
)
util = s.util_df[
    ["name", "points", "exp_ps_sp_pa", "sp_split", "fd_salary"]
