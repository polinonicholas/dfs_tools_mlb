...
class FDSlateSingle:
    ...
    def entries_df(self, reset=False):
        ...
        if path.exists() and not reset:
            ...
            cols = ['entry_id', 'contest_id', 'contest_name', 'MVP - 2X Points', 'STAR - 1.5X Points', 'UTIL', 'UTIL.1', 'UTIL.2']
        ...
        while lus > 0:
            ...
            for y,z in enumerate(lineup):
                if not z:
                    ...
                    try:
                        hitters = h[stack_filt & dupe_filt & count_filt & sal_filt & order_filt & plat_filt]
                        hitter = hitters.loc[hitters[non_stack_key].idxmax()]
                    except (KeyError, ValueError):
                        try:
                            hitters = h[stack_filt & dupe_filt & count_filt & sal_filt & order_filt]
                            hitter = hitters.loc[hitters[non_stack_key].idxmax()]
                        except (KeyError, ValueError):
                            try:
                                hitters = h[stack_filt & dupe_filt & count_filt & sal_filt & plat_filt]
                                hitter = hitters.loc[hitters[non_stack_key].idxmax()]
                            except (KeyError, ValueError):
                                try:
                                    hitters = h[stack_filt & dupe_filt & count_filt & order_filt & plat_filt]
                                    hitter = hitters.loc[hitters[non_stack_key].idxmax()]
                                except:
                                    hitters = h[stack_filt & dupe_filt & sal_filt & order_filt & plat_filt]
                                    hitter = hitters.loc[hitters[non_stack_key].idxmax()]
            ...
            while not reset and lineup[0:2] + sorted(lineup[2:5]) in sorted_lus:
                ...
                try:
                    ...
                    hitters = h[dupe_filt & sal_filt & used_filt & count_filt & stack_filt & order_filt & plat_filt]
                    hitter = hitters.loc[hitters[non_stack_key].idxmax()]
