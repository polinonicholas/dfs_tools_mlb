...
def get_statcast_longterm(seasons=[], player_group="", player_ids=[]):
    ...
    for season in seasons:
        ...
        if h:
            ...
            call = statsapi.get(
                "people",
                {"personIds": player_ids, "hydrate": hydrate, "fields": fields},
                force=True,
            )
...
def get_statcast_h(player_id, season):
    ...
    for x in call["people"][0]["stats"][0]["splits"]:
        ...
        if play_id in play_ids:
            ...
            play[description] = x["stat"]["metric"]["value"]
            play["result"] = (
                x["stat"].get("event", {}).get("details", {}).get("event", "")
            )
            play["date"] = x["date"]
...
def get_splits_h(seasons, sport=1, pool="ALL", get_all=True):
    ...
    hydrate = "person"
    if len(seasons) > 1:
        final_path = settings.ARCHIVE_DIR.joinpath(
            pickle_path(name="h_data_" + str(seasons[0]) + "-" + str(seasons[-1]))
        )
        if str(seasons[-1]) == cs and len(seasons) > 2:
            static_path = settings.ARCHIVE_DIR.joinpath(
                pickle_path(name="h_data_" + str(seasons[0]) + "-" + str(seasons[-2]))
            )
        elif str(seasons[-1]) == cs:
            static_path = settings.ARCHIVE_DIR.joinpath(
                pickle_path(name="h_data_" + str(seasons[0]))
            )
        else:
        ...
        for split in splits:
            ...
            for p in data:
                ...
                if player_id not in player_ids:
                    ...
                    current_player = next(
                        x for x in players if x["mlb_id"] == player_id
                    )
...
def get_splits_p(seasons, sport=1, pool="ALL", get_all=True):
    ...
    if len(seasons) > 1:
        final_path = settings.ARCHIVE_DIR.joinpath(
            pickle_path(name="p_data_" + str(seasons[0]) + "-" + str(seasons[-1]))
        )
        if str(seasons[-1]) == cs and len(seasons) > 2:
            static_path = settings.ARCHIVE_DIR.joinpath(
                pickle_path(name="p_data_" + str(seasons[0]) + "-" + str(seasons[-2]))
            )
        elif str(seasons[-1]) == cs:
            static_path = settings.ARCHIVE_DIR.joinpath(
                pickle_path(name="p_data_" + str(seasons[0]))
            )
        else:
            static_path = final_path
    else:
    ...
    for season in seasons:
        ...
        for split in splits:
            if split == "vr":
                hydrate = (
                    f"person(stats(group=[pitching],type=[season],season={season}))"
                )
                call = statsapi.get(
                    "stats",
                    {
